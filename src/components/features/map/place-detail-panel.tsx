...
export function PlaceDetailPanel({ place, onClose, variant = 'panel' }: PlaceDetailPanelProps) {
  ...
  const isRestaurant = place.category === 'restaurant' ||
    /restaurant|cafe|bakery|bar|food|meal/.test(place.type ?? '');

  useEffect(() => {
    if (fetchedForId.current === place.id) return;
    fetchedForId.current = place.id;
      ...
      <div className="overflow-y-auto p-6 space-y-4 flex-1">
        {activeTab === 'overview' ? (
          <>
            ...
            {/* Top menu items (restaurants only) — only shown after user triggers AI */}
