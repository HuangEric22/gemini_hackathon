...
export function buildTravelMatrixBlock(matrix: TravelMatrix, mode: string): string {
  const lines: string[] = [];
...
export function buildDayAssignmentsBlock(assignments: Record<string, string[]>): string {
  const lines: string[] = [];
...
// Builds the final structured-output prompt after tool context is collected.
export function buildItineraryPrompt(input: GenerateItineraryInput, travelMatrix: TravelMatrix, planningNotes?: string): string {
